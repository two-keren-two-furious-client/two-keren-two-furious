<template>
    <div class="container">
        <div class="row">
            ...
            <div class="card-header">Player Joined</div>
                            ...
                            <div class="row">                               
                                ...
                                <div class="card ml-2" style="width: 18rem; margin: 10px;" v-if="p1">
                                        <div class="card-body" >
                                            ...
                                            <div class="btn btn-success" :class="{disabled: mytoken !== p1.id}" v-if="!p1.ready" @click="statusChanger('p1')"> Ready</div>
                                            <div class="btn btn-danger" :class="{disabled: mytoken !== p1.id}" v-else @click="statusChanger('p1')">Cancel ready</div>        
                                        </div>
                                     </div>
                                </div>
                                 <!-- p2 -->
          ...
          if (Object.values(snapshot.val())[i].id === token) {
            if (Object.keys(snapshot.val())[i] === "p1") {
              ...
              db.ref(
                `/db/rooms/` +
                  roomId +
                  `/player/${Object.keys(snapshot.val())[i]}`
              ).remove();
              localStorage.removeItem("roomId");
